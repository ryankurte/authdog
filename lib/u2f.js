...
exports.finishRegistration = function(registrationRequest, registrationResponse){
  ...
  if (!registrationResponse.clientData) { 
    return Promise.reject({errorCode: errors.registration.NO_CLIENT_DATA, message: "No client data"});
  }
  ...
  if (clientData.challenge !== registrationRequest.challenge) { 
    return Promise.reject({errorCode: errors.registration.CHALLENGE_MISMATCH, message: "Challenge mismatch"});
  }

  // Then, the "registrationData" part.
  if (!registrationResponse.registrationData) { 
    return Promise.reject({errorCode: errors.registration.NO_REGISTRATION_DATA, message: "No registration data"});
  }
  ...
  if (reservedByte !== 0x05) { 
    return Promise.reject({errorCode: errors.registration.WRONG_RESERVED_BYTE, message: "Incorrect reserved byte"});
  }
  ...
  if (!tmpKey) { 
    return Promise.reject({errorCode: errors.registration.PUBLIC_KEY_ERROR, message: "Public key error"});
  }
  ...
  if(!is_valid) { 
    return Promise.reject({errorCode: errors.registration.SIGNATURE_INVALID, message: "Signature invalid"});
  }
 ...
 * the server.
 */
exports.startAuthentication = function(appId, keyHandle){
  var challenge = generateChallenge();
  return {
    version: U2F_VERSION,
    appId: appId,
    // Apprently, the client-side library uses the RFC4648 flavour of base64
    keyHandle: base64_to_RFC4648(keyHandle),
    challenge: base64_to_RFC4648(challenge)
  };
};
 ...
 */
exports.finishAuthentication = function(challenge, deviceResponse, publicKey){
  // Validation
  if (!deviceResponse.keyHandle) { 
    return Promise.reject({errorCode: errors.authentication.NO_KEY_HANDLE, message: "Response must contain key handle"}); 
  }
  if (!deviceResponse.clientData) { 
    return Promise.reject({errorCode: errors.authentication.NO_CLIENT_DATA, message: "Response must contain client data"}); 
  }
  if (!deviceResponse.signatureData) { 
    return Promise.reject({errorCode: errors.authentication.NO_SIGNATURE_DATA, message: "Response must contain signature data"}); 
  }
  if (deviceResponse.keyHandle !== challenge.keyHandle) { 
    return Promise.reject({errorCode: errors.authentication.WRONG_KEY_HANDLE, message: "Invalid key handle"}); 
  }
  ...
  /*
  if (!clientData.typ) { 
    return Promise.reject({errorCode: errors.authentication.NO_TYPE, message: "Client data should contain type specifier"}); 
  }
  if (clientData.typ !== "navigator.id.getAssertion") { 
    return Promise.reject({errorCode: errors.authentication.WRONG_TYPE, message: "Incorrect type specified in client data"}); 
  }
  */
  if (clientData.challenge !== challenge.challenge) { 
    return Promise.reject({errorCode: errors.authentication.CHALLENGE_MISMATCH, message: "Authentication challenge mismatch"}); 
  }
  ...
  if(!is_valid) { 
    return Promise.reject({errorCode: errors.authentication.SIGNATURE_INVALID, message: "Invalid authentication signature"}); 
  }
