...
exports.finishRegistration = function(registrationRequest, registrationResponse){
  ...
  if (!registrationResponse.clientData) { 
    return {errorCode: errors.registration.NO_CLIENT_DATA, message: "No client data"};
  }
...
*/
  ...
  if (clientData.challenge !== registrationRequest.registerRequests[0].challenge) { 
    return {errorCode: errors.registration.CHALLENGE_MISMATCH, message: "Challenge mismatch"};
  }

  // Then, the "registrationData" part.
  if (!registrationResponse.registrationData) { 
    return {errorCode: errors.registration.NO_REGISTRATION_DATA, message: "No registration data"};
  }
  ...
  if (reservedByte !== 0x05) { 
    return {errorCode: errors.registration.WRONG_RESERVED_BYTE, message: "Incorrect reserved byte"};
  }
  ...
  if (!tmpKey) { 
    return {errorCode: errors.registration.PUBLIC_KEY_ERROR, message: "Public key error"};
  }
  ...
  if(!is_valid) { 
    return {errorCode: errors.registration.SIGNATURE_INVALID, message: "Signature invalid"};
  }
 ...
 * the server.
 */
exports.startAuthentication = function(appId, keyHandles, options){
  var options = options || {};

  if(typeof appId === 'undefined') {
    return {errorCode: 0, message: "Authentication request requires appId"};
  }

  if(typeof keyHandles === 'undefined') {
    return {errorCode: 0, message: "Authentication request requires keyHandles"};
  }
  ...
  for(var index in keyHandles) {
    request.registeredKeys.push({version: U2F_VERSION, keyHandle: keyHandles[index]});
  }
...
exports.finishAuthentication = function(challenge, deviceResponse, publicKey){
  ...
  if (!deviceResponse.keyHandle) { 
    return {errorCode: errors.authentication.NO_KEY_HANDLE, message: "Response must contain key handle"}; 
  }
  if (!deviceResponse.clientData) { 
    return {errorCode: errors.authentication.NO_CLIENT_DATA, message: "Response must contain client data"}; 
  }
  if (!deviceResponse.signatureData) { 
    return {errorCode: errors.authentication.NO_SIGNATURE_DATA, message: "Response must contain signature data"}; 
  }

  // Check response key handle was issued in the challenge
  var usedKey = challenge.registeredKeys.find(function(item) {
    return item.keyHandle == deviceResponse.keyHandle;
  }) || null;
  if (usedKey == null) { 
    return {errorCode: errors.authentication.WRONG_KEY_HANDLE, message: "Invalid key handle"}; 
  }
  ...
  /* Not sure where these types come from..?
  if (!clientData.typ) { 
    return {errorCode: errors.authentication.NO_TYPE, message: "Client data should contain type specifier"}; 
  }
  if (clientData.typ !== "navigator.id.getAssertion") { 
    return {errorCode: errors.authentication.WRONG_TYPE, message: "Incorrect type specified in client data"}; 
  }
  */
  if (clientData.challenge !== challenge.challenge) { 
    return {errorCode: errors.authentication.CHALLENGE_MISMATCH, message: "Authentication challenge mismatch"}; 
  }
  ...
  if(!is_valid) { 
    return {errorCode: errors.authentication.SIGNATURE_INVALID, message: "Invalid authentication signature"}; 
  }
